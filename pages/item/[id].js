        ...
        {item && (
          <>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-8">
              ...
              {item.url && (
                <a href={addUtmSource(item.url)} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline text-sm break-words">
                  {item.url}
                </a>
              )}
              {item.text && <div className="mt-4 text-sm" dangerouslySetInnerHTML={{ __html: item.text }} />}
            </div>
